...
export function Sidebar({
  ...
  return (
    <aside className={cn(
      ...
      <div className="p-5 border-b border-slate-800 min-w-[280px]">
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6 min-w-[320px]">
        {Object.entries(groupedLessons).map(([phase, modules]) => (
          <div key={phase} className="space-y-4">
            <h2 className="text-[10px] font-bold text-blue-500 uppercase tracking-[0.2em] px-2">
              {phase}
            </h2>
              ...
              {Object.entries(modules).map(([moduleName, moduleLessons]) => {
                ...
                return (
                  <div key={moduleKey} className="space-y-1">
                    <button onClick={() => toggleModule(moduleKey)} className={cn("w-full flex items-center justify-between p-3 rounded-xl transition-all", isExpanded ? "bg-slate-800/60 text-slate-200" : "hover:bg-slate-800/30 text-slate-400")}>
                      <span className="text-xs font-bold uppercase truncate">{moduleName}</span>
                      {isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    </button>
                    {isExpanded && (
                      <div className="ml-4 pl-3 border-l border-slate-800 space-y-1 mt-1">
                        {moduleLessons.map((l: any) => (
                          <button key={l.id} onClick={() => onSelectLesson(l.originalIndex)} className={cn("w-full text-left p-2 rounded-lg text-xs font-medium", currentLessonIndex === l.originalIndex ? "text-blue-400" : "text-slate-500 hover:text-slate-300")}>
                            {l.title}
                          </button>
                        ))}
                      </div>
